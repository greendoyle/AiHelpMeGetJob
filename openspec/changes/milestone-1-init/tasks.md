...
- [x] 5.1 在Application初始化MMKV（在Application.onCreate中调用MMKV.initialize(context)）
- [x] 5.2 创建mmkv/StorageManager.kt单例，封装基础类型get/set方法（String、Boolean、Int、Long）
- [x] 5.3 封装复杂对象存储：通过Gson序列化data class为JSON字符串后存入MMKV
- [x] 5.4 封装delete和clear方法
- [x] 5.5 验收：存储简历信息后重启APP，确认数据正确读取

## 6. 网络请求封装（Network Client）

- [x] 6.1 创建network/ApiClient.kt单例，配置OkHttp（超时时间30秒、添加LoggingInterceptor仅debug构建）
- [x] 6.2 配置Retrofit实例：GsonConverterFactory、CoroutineCallAdapterFactory
- [x] 6.3 创建OpenAI兼容API接口定义（POST /chat/completions，请求体含model/messages/temperature）
- [x] 6.4 封装动态baseUrl功能（支持用户配置的API URL切换）
- [x] 6.5 封装API连通性测试方法（轻量请求验证配置有效性）
- [x] 6.6 验收：使用测试API URL确认能成功发起请求并解析返回结果

## 7. 权限引导与检测（Permission Helper）

- [x] 7.1 创建permission/PermissionHelper.kt工具类
- [x] 7.2 实现无障碍服务权限检测方法（遍历Settings.Secure检查服务是否开启）
- [x] 7.3 实现悬浮窗权限检测方法（Settings.canDrawOverlays）
- [x] 7.4 实现跳转无障碍服务设置页方法（Intent跳转ACTION_ACCESSIBILITY_SETTINGS）
- [x] 7.5 实现跳转悬浮窗权限设置页方法（Intent跳转ACTION_MANAGE_OVERLAY_PERMISSION）
- [x] 7.6 验收：APP启动检测权限状态，点击引导按钮能跳转至对应系统设置页

## 8. 集成验收

- [x] 8.1 编译项目，确认无编译报错、无警告
- [x] 8.2 安装到Android 10.0及以上真机/模拟器，确认启动无崩溃
